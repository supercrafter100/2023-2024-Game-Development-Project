using System;
using GameDevProject.Core.animations.enemy2;
using GameDevProject.Core.gameStates;
using GameDevProject.utility;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using DeathState = GameDevProject.Core.animations.enemy2.DeathState;
...
public class Enemy2 : Enemy
{
    ...
    public Enemy2(GameManager game, Vector2 startPosition, Vector2 endPosition)
    {
        LogPrefix = "[E1]";
        LogChanges = true;
        ...
        Texture = _game.RootGame.Content.Load<Texture2D>("enemy2");
    }
    ...
    // - If character is less than a certain amount away from the player, the enemy will move towards the player their X coordinate
    ...
    public override void Update(GameTime time)
    {
        ...
        {
            if (Vector2.Distance(Position, _game.Character.Position) < 50)
            {
            ...
            if (_game.Character.Position.X > Position.X && Position.X < _endPos.X)
            {
                Direction = Direction.Right;
                Position = new Vector2(Position.X + 2.5f, Position.Y);
            } else if (_game.Character.Position.X < Position.X && Position.X > _startPos.X)
            {
